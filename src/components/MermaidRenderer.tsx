...
const MermaidRenderer: React.FC<MermaidRendererProps> = ({ chart }) => {
  ...
  const chartId = useRef(`mermaid-chart-${Math.random().toString(36).substring(7)}`);
  ...
  const adjustSvgSize = React.useCallback((svg: string): string => {
  ...
  useEffect(() => {
    ...
    // Render diagram
    mermaid.render(chartId.current, sanitizedChart)
      .then(({ svg }) => {
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="fixed inset-0 bg-[#0c0c0c] flex items-center justify-center z-50 p-4">
          <div className="bg-[#1e1e1e] rounded-lg w-full max-w-6xl h-auto max-h-[90vh] overflow-auto relative">
            {/* Close button */}
            {/* <button 
            ...
            <div className="px-4 py-3 border-b border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-white">Diagram Preview</h3>
              
              {/* Scale controls */}
              <div className="flex items-center space-x-2">
                <span className="text-gray-400 text-sm mr-2">{Math.round(previewScale * 100)}%</span>
                <button 
            ...
            <div className="p-6 flex items-center justify-center overflow-auto" style={{ minHeight: "500px" }}>
              <div ref={expandedContainerRef} className="mermaid-expanded-container"></div>
            </div>
          </div>
        </div>
      )}
