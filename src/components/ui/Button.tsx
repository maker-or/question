...
const buttonVariants = cva(
  ...
  "relative flex items-center justify-center rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none",
  {
    variants: {
      variant: {
        primary: "bg-gradient-to-br from-[#0D0C0C] to-[#252525] text-white hover:scale-105 active:scale-95 transform",
        secondary: "bg-gradient-to-br from-[#e2e2e2] to-[#d0d0d0] text-[#0c0c0c] hover:scale-105 active:scale-95 transform",
        accent: "bg-gradient-to-br from-[#FF5E00] to-[#FF8A3D] text-white hover:scale-105 active:scale-95 transform",
        outline: "border border-gray-200 dark:border-gray-700 bg-transparent dark:text-white text-[#0c0c0c] hover:bg-gray-100 dark:hover:bg-[#323232]",
        ghost: "bg-transparent dark:text-white text-[#0c0c0c] hover:bg-gray-100 dark:hover:bg-[#323232]",
        ...
        action: "p-3 rounded-full hover:bg-[#323232] text-[#f7eee3] transition-colors duration-200",
      },
      size: {
        sm: "h-8 px-3 text-sm",
    ...
    compoundVariants: [
      // Compound variants for inner shadow effects based on variant
      {
        ...
        className: "before:bg-gradient-to-br before:from-[#252525] before:to-[#0D0C0C] shadow-[inset_0px_-4px_6px_rgba(0,0,0,0.3),0px_4px_15px_rgba(0,0,0,0.25)] hover:shadow-[0_0_15px_rgba(0,0,0,0.5)]",
      },
      {
        variant: "secondary",
        withInnerShadow: true,
        className: "before:bg-gradient-to-br before:from-[#d0d0d0] before:to-[#e2e2e2] shadow-[inset_0px_-4px_6px_rgba(0,0,0,0.1),0px_4px_10px_rgba(0,0,0,0.1)] hover:shadow-[0_0_15px_rgba(0,0,0,0.2)]",
      },
      {
        variant: "accent",
        withInnerShadow: true,
        className: "before:bg-gradient-to-br before:from-[#FF8A3D] before:to-[#FF5E00] shadow-[inset_0px_-4px_6px_rgba(0,0,0,0.2),0px_4px_15px_rgba(0,0,0,0.15)] hover:shadow-[0_0_15px_rgba(255,94,0,0.5)]",
      },
    ],
    defaultVariants: {
      variant: "primary",
...
const getInnerEffects = (variant: string | null | undefined) => {
  if (!variant || variant === 'outline' || variant === 'ghost' || variant === 'link') return null;
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, isRounded, withInnerShadow, leftIcon, rightIcon, isLoading, children, ...props }, ref) => {
    const buttonClasses = cn(
      buttonVariants({ variant, size, isRounded, withInnerShadow, className }),
      props.disabled ? "opacity-50 cursor-not-allowed" : "",
      isLoading ? "relative text-transparent hover:scale-100 active:scale-100" : "",
