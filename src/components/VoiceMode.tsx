...
const VoiceMode: React.FC<VoiceModeProps> = ({ onSubmit, onExit, lastResponse }) => {
  const [isRecording, setIsRecording] = useState(false);
  const [transcribedText, setTranscribedText] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [visualizerValues, setVisualizerValues] = useState<number[]>(Array(30).fill(5));
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (data.useBrowserTTS) {
          console.log("Using browser's speech synthesis as instructed by server");
      ...
      setError(`Failed to play audio response: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    try {
      ...
      utterance.onerror = (event) => {
        ...
        const errorDetails = {
          ...
          isCancelled: window.speechSynthesis.pending === false && window.speechSynthesis.speaking === false
        };
        ...
        if (errorDetails.isCancelled) {
          console.log("Speech was cancelled - this is expected during normal operation");
        } else {
          setError(`Speech playback error: ${errorDetails.error}. ${errorDetails.message}`);
        }
      ...
      const timeoutDuration = Math.min(120000, Math.max(10000, text.length * 100));
      ...
      setError(`Browser speech synthesis error: ${e instanceof Error ? e.message : 'Unknown error'}`);
      setIsSpeaking(false);
    }
  };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(errorData.error || `Chat request failed: ${response.status} ${response.statusText}`);
        } catch (jsonError) {
      ...
      const responseTimeout = setTimeout(() => {
        if (!hasReceivedData && isProcessing) {
          console.error("Response timeout - no data received within timeout period");
          setError("Response timeout - no data received from server");
          setIsProcessing(false);
          
          // Add fallback message to conversation history
          const fallbackMessage = "I'm sorry, I couldn't generate a response at this time. Please try again.";
          setConversationHistory([
            ...updatedHistory,
      ...
      try {
              ...
              for (const line of lines) {
                ...
                if (trimmedLine.startsWith('data: ')) {
                  ...
                  try {
                    ...
                    console.warn(`JSON parse error in data line: ${parseError}. Content: ${content.substring(0, 50)}...`);
                ...
                // Case 3: JSON object without SSE formatting
                  ...
                  try {
                    ...
                    const contentMatch = trimmedLine.match(/"content":"([^"]*)"/) || 
        ...
        if (!hasReceivedData) {
          throw new Error(`Failed to process stream: ${streamError instanceof Error ? streamError.message : 'Unknown error'}`);
        }
      } finally {
      ...
      if (cleanedText) {
        ...
        setError("No response received from server. I've provided a fallback response instead.");
      }
    } catch (error) {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            console.error('Transcription failed:', response.status, response.statusText, errorData);
            throw new Error(`Transcription failed: ${response.statusText}${errorData ? ` - ${errorData.error}` : ''}`);
          }
          
          const data = await response.json();
          const transcribed = data.text || "Sorry, I couldn't transcribe that. Please try again.";
          ...
          if (transcribed && transcribed !== "Sorry, I couldn't transcribe that. Please try again.") {
    ...
    <div className="flex flex-col items-center justify-center space-y-8 w-full max-w-lg p-8">
      ...
      <div className="relative flex items-center justify-center w-full">
        ...
        <div 
          ...
          style={{
            background: 'linear-gradient(49.71deg, #000000 16.4%, #FF5E00 111.47%)',
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex items-end justify-center h-20 gap-[2px] px-2 w-full">
                {visualizerValues.map((value, index) => (
                  <div 
                    key={index}
                    style={{ height: `${value}px`, backgroundColor: '#FF5E00' }}
