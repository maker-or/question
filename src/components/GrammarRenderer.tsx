...
const GrammarRenderer: React.FC<GrammarRendererProps> = ({ grammar }) => {
  ...
  const processedRules = rules.map(rule => {
    ...
    if (rule.includes('|')) {
      return { left: '', alternatives: rule.split('|').map(alt => alt.trim()) };
    }
  ...
  return (
    <div className="grammar-renderer my-6 overflow-x-auto">
      <table className="grammar-table min-w-fit border-collapse text-lg">
        <tbody>
          {processedRules.map((rule, ruleIndex) => (
            <tr key={ruleIndex} className="border-b border-gray-200 dark:border-gray-700 last:border-0">
              <td className="py-2 pr-3 text-right font-medium min-w-[3rem] text-gray-800 dark:text-gray-200">
                {rule.left}
              </td>
              ...
              <td className="py-2 pl-3">
                {rule.alternatives.map((alt, altIndex) => (
                  <div key={altIndex} className="grammar-alternative">
                    {altIndex > 0 && (
                      <span className="mx-2 text-gray-500 dark:text-gray-400 font-medium">|</span>
                    )}
                    <span className="grammar-symbols">
                      {alt.split(/\s+/).map((symbol, symIndex) => {
                        // Special formatting for different symbol types
                        if (symbol === 'ε') {
                          return (
                            <span key={symIndex} className="mx-1 italic text-gray-500 dark:text-gray-400">
                              ε
                            </span>
                          );
                          ...
                          return (
                            <span key={symIndex} className="mx-1 font-medium text-green-600 dark:text-green-400">
                              {symbol}
                            </span>
                          );
                        } else if (/^[a-z]+$/.test(symbol)) {
                          ...
                          return (
                            <span key={symIndex} className="mx-1 font-mono text-purple-600 dark:text-purple-400">
                              {symbol}
                            </span>
                          );
                        } else if (/^[+*()^]$/.test(symbol)) {
                          ...
                          return (
                            <span key={symIndex} className="mx-1 font-bold text-orange-600 dark:text-orange-400">
                              {symbol}
                            </span>
                          );
                        } else {
                          // Other symbols
                          return (
                            <span key={symIndex} className="mx-1">
                              {symbol}
                            </span>
                          );
                        }
