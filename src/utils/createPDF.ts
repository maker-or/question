...
export const createPDF = (messages: Message[]): void => {
  ...
  const formatMarkdownForPDF = (content: string): FormattedContent => {
    ...
    // Process inline code (content within backticks)
    processedContent = processedContent.replace(/`([^`]+)`/g, (_, code: string) => {
  ...
  // Helper function to render a code block - updated styling
  const renderCodeBlock = (
    ...
    // Split code while preserving indentation and whitespace
    const codeLines = code.split('\n');
    const lines: string[] = [];
    
    // Process each line to preserve indentation but wrap if too long
    for (const line of codeLines) {
      // If line is short enough, keep it as is
      if (doc.getStringUnitWidth(line) * 11 / doc.internal.scaleFactor <= maxWidth - 30) {
        lines.push(line);
      } else {
        // For long lines, wrap while trying to preserve indentation
        const indentMatch = line.match(/^(\s+)/);
        const indent = indentMatch ? indentMatch[1] : '';
        const wrappedLines = doc.splitTextToSize(line, maxWidth - 30);
        
        // Add first line as is
        lines.push(wrappedLines[0]);
        
        // Add indent to continuation lines
        for (let i = 1; i < wrappedLines.length; i++) {
          lines.push(indent + '  ' + wrappedLines[i]); // Add extra indent for continuation
        }
      }
    }
    ...
    // Calculate expected height for initial background
    const initialHeight = Math.min(
      lines.length * LINE_HEIGHT + 34,
      pageHeight - y - margin
    );

    // Initial code block background
    doc.setFillColor(42, 46, 54);
    doc.roundedRect(startX - 12, y - 12, maxWidth + 24, initialHeight, 8, 8, "F");
    ...
    if (lang && lang !== "text") {
      ...
      doc.roundedRect(startX, y - 5, 70, 22, 4, 4, "F");
      doc.setFont("Courier", "bold");
      doc.setFontSize(12);
      doc.setTextColor(130, 170, 255);
      doc.text(lang.toUpperCase(), startX + 10, y + 10);
      y += 24;
    } else {
      y += 20;
    }
    ...
    // Add line numbers for code blocks with many lines
    const showLineNumbers = lines.length > 5;
    if (showLineNumbers) {
      lineNumberWidth = 20; // Width reserved for line numbers
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (y > pageHeight - margin) {
        ...
        const remainingHeight = Math.min(
          (lines.length - i) * LINE_HEIGHT + 34, 
          pageHeight - y - margin
        );
        
        doc.setFillColor(42, 46, 54);
        doc.roundedRect(startX - 12, y - 12, maxWidth + 24, remainingHeight, 8, 8, "F");
        ...
        doc.setFillColor(32, 36, 44);
        doc.roundedRect(startX, y - 5, 120, 22, 4, 4, "F");
        doc.setFont("Courier", "bold");
        doc.setFontSize(10);
        doc.setTextColor(130, 170, 255);
      ...
      if (showLineNumbers) {
        ...
        doc.setFontSize(9);
        doc.text(lineNumber.toString(), startX + 5, y);
        doc.setTextColor(230, 230, 230);
        doc.setFontSize(11);
        lineNumber++;
      }
  ...
  const renderTable = (
    ...
    const colWidth = Math.min(150, (maxWidth / colCount));
    let y = startY + 10;
    
    // Add table border
    doc.setDrawColor(200, 200, 200);
    doc.roundedRect(startX - 5, y - 15, maxWidth + 10, 
                   (tableData.length * (LINE_HEIGHT + 8)) + 10, 5, 5, "S");
    
    // Draw header row with better styling
    if (tableData[0]) {
      doc.setFillColor(255, 98, 0, 0.1); // Light orange background for header
      doc.rect(startX - 4, y - 14, maxWidth + 8, LINE_HEIGHT + 10, "F");
      ...
      for (let c = 0; c < tableData[0].length; c++) {
        ...
        doc.line(cellX + colWidth, y - 14, cellX + colWidth, y + LINE_HEIGHT - 4);
        
        doc.text(cell.toString().substring(0, 18) + (cell.length > 18 ? "..." : ""), 
                cellX + 5, y);
      }
      y += LINE_HEIGHT + 8;
    }
    ...
    for (let r = 1; r < tableData.length; r++) {
      ...
      if (r % 2 === 0) {
        ...
        doc.rect(startX - 4, y - 9, maxWidth + 8, LINE_HEIGHT + 8, "F");
      }
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        // Add cell border
        doc.setDrawColor(220, 220, 220);
        doc.line(cellX + colWidth, y - 9, cellX + colWidth, y + LINE_HEIGHT - 1);
        if (r === tableData.length - 1) {
          doc.line(startX - 4, y + LINE_HEIGHT - 1, startX + maxWidth + 4, y + LINE_HEIGHT - 1);
        }
  ...
  const renderStyledText = (
    ...
    //   return y + 16; // Better spacing after horizontal rule
    // }
  ...
  messages.forEach((message: Message, index: number) => {
    ...
    // Better visual distinction between messages
    // Add message role indicator with improved styling
    if (message.role === "user") {
      // User message styling with subtle background
      // doc.setFillColor(250, 246, 240); // Very light orange background
      // doc.roundedRect(margin - 20, y - 15, pageWidth - margin * 2 + 40, 36, 8, 8, "F");
      doc.setFont("Helvetica", "bold");
      doc.setFontSize(24);
      doc.setTextColor(255, 98, 0); // Orange color
      doc.text(message.content, margin, y + 5);
    } 
    y += 40; // Better spacing after message header
    ...
    // Process message content with enhanced formatting
    ...
    let prevWasSpecialBlock = false; // Track if previous element was a special block
    
    for (const part of parts) {
      // Add extra spacing between different block types for better readability
      if ((part.includes('__CODE_BLOCK_') || part.includes('__MATH_BLOCK_') || 
           part.match(/^<<H1>>.*<<H1>>$/) || part.match(/^<<H2>>.*<<H2>>$/)) && 
          !prevWasSpecialBlock) {
        y += 8; // Extra spacing before special blocks
      }
      
      if (part.includes('__CODE_BLOCK_')) {
        const blockIndexMatch: RegExpMatchArray | null = part.match(/__CODE_BLOCK_(\d+)__/);
        if (blockIndexMatch) {
          const blockIndex: number = parseInt(blockIndexMatch[1]);
          if (blockIndex >= 0 && blockIndex < codeBlocks.length) {
            const { code, lang } = codeBlocks[blockIndex];
            y = renderCodeBlock(doc, code, lang, margin, y, pageWidth - margin * 2);
            prevWasSpecialBlock = true;
          }
        }
      } else if (part.includes('__MATH_BLOCK_')) {
        const mathIndexMatch: RegExpMatchArray | null = part.match(/__MATH_BLOCK_(\d+)__/);
        if (mathIndexMatch) {
          const mathIndex: number = parseInt(mathIndexMatch[1]);
          const { math } = mathExpressions[mathIndex];
          y = renderMathBlock(doc, math, margin, y, pageWidth - margin * 2);
          prevWasSpecialBlock = true;
        }
      } else if (part.trim() === '') {
        y += LINE_HEIGHT / 2; // Better spacing for empty lines
      ...
      // Check if we need a new page with improved page transitions
      if (y > pageHeight - margin - 40) {
        doc.addPage();
        y = margin + 20; // Better top margin on new pages
      }
    }

    y += 20; // Increased space between messages
