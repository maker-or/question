...
const renderTable = (
  ...
  let y = startY + 20;
  const tableWidth = colWidths.reduce((sum, w) => sum + w, 0) + (colCount * 8);
  ...
  for (let r = 1; r < tableData.length; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      const cell = row[c] || "";
      const cellLines = doc.splitTextToSize(cell, colWidths[c] - 14);
        ...
        if (part.includes('__CODE_BLOCK_')) {
          ...
          if (tableIndexMatch) {
            ...
            if (tableIndex >= 0 && tableIndex < tables.length) {
              y += 8;
              y = renderTable(doc, tables[tableIndex], margin, y, pageWidth - margin * 2);
              prevWasSpecialBlock = true;
            }
          }
          ...
          if (part.trim().startsWith('__INLINE_CODE_') && part.trim().endsWith('__')) {
            ...
            if (codeMatch) {
              ...
              if (inlineCode) {
                y = renderCodeBlock(doc, inlineCode.code, "", margin, y, pageWidth - margin * 2);
                prevWasSpecialBlock = true;
                continue;
              }
            }
