        ...
        <form onSubmit={onSubmit} className="mt-4 w-full px-4 pb-4">
          <div className="relative flex-row items-center w-full">
            ...
            <button
              ...
              className="p-3 rounded-lg bg-[#5F54FF] text-[#f7eee3] w-full
               transition-colors 
            ...
            >
              {m.role === 'user' ? (
                <div className="flex items-start gap-4 w-full">
                  <div className="max-w-xl text-[2rem] text-[#f7eee3bb] font-serif italic tracking-tight rounded-xl p-4 w-full">
                    <h1 className="whitespace-pre-wrap w-full">{m.content}</h1>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-start gap-2 w-full">
                  <div className="max-w-screen-lg tracking-tight text-[#f7eee3fd] text-[1.4rem] rounded-xl p-4 bg-[#2a2a2a] shadow-md w-full relative">
                    <p className="whitespace-pre-wrap">{m.content}</p>
                    <button 
                      onClick={() => copyMessage(m.content, m.id)}
                      className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[#f7eee3] hover:text-white"
                    >
