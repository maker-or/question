...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const selectedModel = body.model || "openrouter/horizon-beta";
    ...
    try {
      ...
      try {
        decision = await withTimeout(
          generateText({
            model:
              openrouter("openrouter/horizon-beta") || groq("qwen-qwq-32b"),
        ...
        try {
          ...
          try {
            ...
            if (!queryResponse.matches || queryResponse.matches.length === 0) {
              console.log(
                "No matches found in Pinecone, falling back to general knowledge",
              );
              finalPrompt = `
                ${conversationContext}
                Question: ${query}
        ...
        try {
          ...
          const isRateLimitError =
            ...
            (streamError instanceof Error &&
              ...
              typeof (streamError.cause as { message: string }).message ===
                "string" &&
              ((streamError.cause as { message: string }).message.includes(
                "Rate limit exceeded",
              ) ||
                (streamError.cause as { message: string }).message.includes(
                  "credits",
          ...
          if (isRateLimitError) {
            ...
            const creditLimitMessage =
              "You've reached your free usage limit for AI models today. Please try again tomorrow or switch to a different model.";
            ...
            const fallbackMessage =
              "I'm sorry, but my response is taking longer than expected to generate. Please try again with a simpler question or try again later.";
            const stream = new ReadableStream({
              start(controller) {
      ...
      const encoder = new TextEncoder();
      const fallbackMessage =
        "I'm sorry, I couldn't connect to the AI services at this time. Please try again later.";
